...
def get_image_path(item_name: str):
    for suffix in [".jpg", ".jpeg", ".png", ".webp"]:
        path = IMAGES_DIR / f"{item_name}{suffix}"
        if path.exists():
            return path
    return None
...
def month_chip_html(text, active=False):
    bg = "#E7F3E5" if active else "#FFFFFF"
    color = "#365C3B" if active else "#6B7280"
    border = "#B8D8B3" if active else "#E5E7EB"
    return f"""
    <div style="
        display:inline-block;
        padding:6px 12px;
...
def render_card(item, month, compact=False):
    ...
    with st.container(border=True):
        if image_path:
            ...
            st.caption(f"未找到图片：images/{item['name']}.*")
        ...
        with top_right:
            ...
            if st.button(
                fav,
                key=f"fav_{month}_{item['name']}_{'compact' if compact else 'full'}",
                help="收藏/取消收藏"
            ):
                toggle_favorite(item["name"])
                st.rerun()

        tags = item.get("tags", [])[:2 if compact else 4]
        if tags:
            st.markdown("".join([pill_html(t) for t in tags]), unsafe_allow_html=True)

        desc = item.get("desc", "")
        if compact and len(desc) > 28:
            desc = desc[:28] + "..."
        st.write(desc)
        ...
        with col1:
            if st.button(
                "查看详情",
                key=f"detail_{month}_{item['name']}_{'compact' if compact else 'full'}"
            ):
                open_detail(item["name"])
                st.rerun()
        with col2:
            btn_text = "收藏" if item["name"] not in st.session_state.favorites else "已收藏"
            if st.button(
                btn_text,
                key=f"save_{month}_{item['name']}_{'compact' if compact else 'full'}"
            ):
                toggle_favorite(item["name"])
                st.rerun()
...
def go_to_query_with_filters(month=None, category="全部", tag="全部", keyword=""):
    if month:
...
with left:
    st.title("应季果蔬")
    st.caption("按月份看看当季推荐，简单找一找适合现在吃什么")
with right:
    page = st.radio(
        "页面",
        ["首页", "查询页", "收藏页", "详情页"],
...
if st.session_state.page == "首页":
    active_month = st.session_state.selected_month if st.session_state.selected_month in data else active_month
    month_data = data[active_month]
    month_items = get_month_items(active_month)

    st.markdown(f"## {active_month}吃什么")
    season_hint = month_data.get("season_hint", "")
    if season_hint:
        st.caption(season_hint)

    st.markdown("### 月份切换")
    chip_html = "".join([month_chip_html(month, month == active_month) for month in months])
    st.markdown(chip_html, unsafe_allow_html=True)
    selected_home_month = st.selectbox(
        "选择月份",
        months,
        index=months.index(active_month),
        label_visibility="collapsed",
        key="home_month_select"
    )
    if selected_home_month != st.session_state.selected_month:
        st.session_state.selected_month = selected_home_month
        st.rerun()
    ...
    with search_col1:
        st.session_state.search_input = st.text_input(
            ...
            placeholder="输入果蔬名称，例如：草莓、菠菜",
            label_visibility="collapsed"
        )
    with search_col2:
        if st.button("搜索"):
            go_to_query_with_filters(month=active_month, keyword=st.session_state.search_input.strip())
            st.rerun()

    st.markdown("## 本月热门")
    hot_items = month_items[:4]
    if hot_items:
        cols = st.columns(2)
    ...
    with a:
        if st.button("当季水果"):
            go_to_query_with_filters(month=active_month, category="水果", tag="全部", keyword="")
            st.rerun()
    with b:
        if st.button("当季蔬菜"):
            go_to_query_with_filters(month=active_month, category="蔬菜", tag="全部", keyword="")
            st.rerun()
    ...
    if tags:
        ...
        for idx, tag in enumerate(tags):
            with cols[idx % 4]:
                if st.button(tag, key=f"home_tag_{tag}"):
                    go_to_query_with_filters(month=active_month, category="全部", tag=tag, keyword="")
                    st.rerun()
...
# -------------------------
    ...
    col1, col2, col3, col4 = st.columns([1.2, 1, 2.2, 0.8])
    with col1:
        selected_month = st.selectbox(
            "月份",
            months,
            index=months.index(st.session_state.selected_month),
            key="query_month_select"
        )
    ...
    with col2:
        selected_category = st.selectbox(
            "分类",
            ["全部", "水果", "蔬菜"],
            index=["全部", "水果", "蔬菜"].index(st.session_state.selected_category)
        )
    ...
    with col3:
        st.session_state.search_input = st.text_input(
            "搜索",
            value=st.session_state.search_input,
            placeholder="搜索果蔬名称或关键词"
        )

    with col4:
        st.write("")
        if st.button("搜索"):
            st.session_state.keyword = st.session_state.search_input.strip()
            st.rerun()
    ...
    for idx, tag in enumerate(month_tags[:10]):
        with tag_cols[idx % 5]:
            ...
            if st.button(label, key=f"filter_{tag}"):
                st.session_state.selected_tag = tag
                st.rerun()
